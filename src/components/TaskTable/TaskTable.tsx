...
const fetchTasks = async (
  ...
  const res = await api.get(
    endpoints.getTasks(projectId, page, limit, query, sortField)
  );
  return res.data;
};
...
const TaskTable: React.FC<TaskTableProps> = ({
  ...
  const {
    ...
    queryFn: () =>
      fetchTasks(projectId, currentPage, limit, debouncedSearch, sort),
      ...
      {tasksData.data.length > 0 ? (
        ...
        <div className="w-full py-16 lg:py-24 flex flex-col gap-4 items-center justify-center text-center bg-gray-50/50 rounded-2xl">
          ...
          <div className="space-y-2">
            ...
            <p className="text-gray-600">
              Assign the first task for this project
            </p>
          </div>
          <CheckUserRole userRole={tasksData.userRole}>
            <button
