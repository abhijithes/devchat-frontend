...
import UploadButton from "../buttons/uploudbtn";
...
export const DetailedTaskView: React.FC<DetailedTaskViewProps> = ({ id }) => {
    ...
    const handleAddComment = async (comment: string) => {
        ...
        if (res.status == 201) {
            ...
            showSnackBar(res.data?.message || "Failed to update comment", "error", 2000);
        }
    };
    ...
    const handleDelete = async (commentId) => {
        ...
        if (res.status === 200) {
            ...
            showSnackBar(res.data?.message || "Failed to delete comment", "error", 2000);
        }
    };
    ...
    const editDescription = useMutation({
        mutationFn: (newDescription: string) =>
            api.patch(endpoints.UpdateTaskDescriptions(id), { description: newDescription }),
        onSuccess: () => {
            showSnackBar("Description updated successfully", "success", 2000);
            queryClient.invalidateQueries({ queryKey: ["taskDetails", id] });
            setFocusDescription(false);
        },
        onError: () => {
            showSnackBar("Failed to update description", "error", 2000);
        },
    });
    ...
    return (
        <div className="pb-8 ">
            ...
            <div className="flex items-top gap-3 mt-4">
                ...
                ></textarea>
                <button onClick={() => setFocusDescription(!focusDescription)} className="mt-2">
                    <Pencil size={20} />
                </button>
            </div>
            {focusDescription && (
                <div className="flex gap-3 mt-2 md:mt-5 mr-8 justify-end">
                    <button
                        className={`input-grad-btn`}
                        onClick={() => setDescription(data?.ticket.taskDescription || "")}
                    >
                        Reset
                    </button>
                    <button className={`input-grad-btn centered`} onClick={handleDiscriptionEdit}>
                        {editDescription.isPending ? <Spinner /> : "Save"}
                    </button>
                </div>
            )}
            <br />
            ...
            <div className="space-y-4">
                ...
                <p className="text-sm text-zinc-500">Shows the current task progress given by project assignee</p>
                {progress || data?.ticket?.status != "not-started" ? (
                    <ProgressBar value={data?.ticket?.status === "completed" ? 100 : progress} />
                ) : (
            ...
            <div>
                ...
                <div className="w-full flex items-center justify-between">
                    ...
                    <button
                        ...
                        className={`${isCommentShow ? "input-grad-btn-cancel" : " input-grad-btn"}`}
                    >
                        {isCommentShow ? <X onClick={() => setSelectedComment(null)} /> : "New"}
                    </button>
                </div>
                <div className="mt-8">
...
export const CommentForm: React.FC<{
    ...
    return (
        ...
        >
            <h1 className="font-medium text-lg mx-3 ">{pre_comment ? "Update" : "Add"} comment</h1>
            <textarea
                value={comment}
