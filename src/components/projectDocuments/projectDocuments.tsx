...
const ProjectDocuments: React.FC<DocumentProps> = ({
  ...
  const getFileIcon = (fileName: string) => {
    ...
    const config =
      iconConfig[extension as keyof typeof iconConfig] || iconConfig.default;
    ...
    <div className={className}>
      <div className={`  grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4`}>
        {documents.map((doc) => (
          <div
            key={doc.fileName}
            className="w-full  bg-gradient-to-tr from-violet-100 to-blue-50 hover:bg-gray-50 rounded-lg border border-gray-200 p-4  hover:border-gray-300 transition-colors duration-200 group"
          >
