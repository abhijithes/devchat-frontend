...
export const Pinnedprojects = ({
  ...
  const handleDrahend = async (e: DragEndEvent) => {
    ...
    const newPins = arrayMove(
      pins,
      getPos(active.id.toString()),
      getPos(over.id.toString())
    );
    setPins(newPins);
  ...
  return (
    <div className="pinned-projects w-full p-3 mx-auto  border border-zinc-100 mt-3  rounded ">
      ...
      {showPinProjects && (
        <SearchProjects
          setShowPinProjects={setShowPinProjects}
          user={user}
          refetch={refetch}
        />
      )}
