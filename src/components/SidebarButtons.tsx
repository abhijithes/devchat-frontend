...
const SidebarButtons: React.FC = () => {
    ...
    return (
        <div className="flex flex-col gap-2 w-full max-h-full relative z-40">
            ...
            <h5 className="font-semibold mt-2 text-sm md:text-base px-2">Your projects</h5>

            {!isLoading && !isError && userProjects.length === 0 && (
                <p className="text-gray-500 px-3 py-2 md:px-4 text-sm">No projects found</p>
            )}
            ...
            {userProjects.map((project, index) => (
                <Link
                    ...
                    className="bg-white text-black px-3 py-2 md:px-4 md:py-2 w-full text-left hover:bg-gray-50 transition-all duration-200 block relative rounded border border-transparent hover:border-gray-200 group"
                >
                    <span className="truncate block text-sm md:text-base">{project.name}</span>
                    ...
                    {/* Hover indicator */}
                    <div className="absolute inset-0 border-2 border-transparent group-hover:border-green-200 rounded pointer-events-none" />
                </Link>
            ))}

            {/* ✅ Assigned Projects Section */}
            <h5 className="font-semibold mt-4 md:mt-6 text-sm md:text-base px-2">Assigned projects</h5>

            {!isLoading && !isError && assignedProjects.length === 0 && (
                <p className="text-gray-500 px-3 py-2 md:px-4 text-sm">No assigned projects</p>
            )}
            ...
            {assignedProjects.map((project) => (
                <Link
                    key={project._id}
                    to={`/project/${project._id}`}
                    className="bg-white text-black px-3 py-2 md:px-4 md:py-2 w-full text-left hover:bg-gray-50 transition-all duration-200 block rounded border border-transparent hover:border-gray-200 group"
                >
                    <span className="truncate block text-sm md:text-base">{project.name}</span>

                    {/* Hover indicator */}
                    <div className="absolute inset-0 border-2 border-transparent group-hover:border-blue-200 rounded pointer-events-none" />
                </Link>
            ))}

            {/* Empty state when no projects at all */}
            {!isLoading && !isError && userProjects.length === 0 && assignedProjects.length === 0 && (
