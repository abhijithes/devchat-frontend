...
const AddProject: React.FC<AddProjectProps> = ({ onClose }) => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const response = await fetch(
        `${endpoints.searchUser}/${id}?search=${encodeURIComponent(searchTerm)}`
      );
  ...
  const uploading =
    fileUploadMutation.isPending || createProjectMutation.isPending;
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (totalFiles > 10) {
      alert(
        `Maximum of 10 files allowed. You currently have ${selectedFiles.length} files selected.`
      );
      return;
    }
