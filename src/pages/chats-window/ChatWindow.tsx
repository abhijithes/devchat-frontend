...
} from "../../components/chats/MessageSkeletons";
...
const ChatWindow = () => {
  ...
  useEffect(() => {
    ...
    socket.on("message_updated", (updated) => {
      queryClient.setQueryData<Message[]>(
        ...
        (old = []) =>
          old.map((msg) =>
            msg._id === updated.id
              ? { ...msg, text: updated.text, isEdited: true }
              : msg
          )
      );
    });
    ...
    socket.on("message_deleted", (deleted) => {
      queryClient.setQueryData<Message[]>(
        ["messages", deleted.roomId],
        (old = []) => old.filter((msg) => msg._id !== deleted._id)
      );
    });
  ...
  const handleFileSelect = async (file) => {
    if (FilePreviews.length + file.length > 4) {
      showSnackBar(
        "You can upload a maximum of 4 Files at a time.",
        "error",
        3000
      );
      return;
          ...
          <div className="flex  items-center justify-between gap-4  border-b border-zinc-300 py-3 ">
            <div className=" flex  items-center gap-4  ">
              ...
              <div className="flex flex-col">
                ...
                <p
                  className={` ${
                    isUserOnline ? "text-green-600" : "text-gray-500"
                  }`}
                >
