...
const SocketBaseProvider = ({ children }: { children: ReactNode }) => {
    ...
    const fetchNotifications = async (): Promise<Notification[]> => {
        try {
            const res = await axios.get(`${endpoints.getNotifications}/?readFilter=${read}`, {
                headers: {
    ...
    useEffect(() => {
        ...
        audioRef.current = new Audio(
            NotificationSounds.find((x) => x.audioId === Number(soundId))?.src ?? NotificationSounds[0].src
        );
        ...
        socket.on("get_notification", (payload: { data: Notification; slug: string; navigationPath: string }) => {
